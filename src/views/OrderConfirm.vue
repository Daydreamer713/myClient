...
// import { currency } from "./../util/currency"; //已经定义全局过滤器
export default {
  data() {
    return {
      subtotal: 0,
  ...
  methods: {
    ...
    payMent() {
      // $route 路由信息,能拿到路由参数 path，params，hash，query，fullPath，matched，name
        ...
        .then((resp) => {
          ...
          if (res.status == 0) {
            this.$router.push({
              ...
              // 教程里这串是 hash 不是 query，可我这里是 query

              // query要用 path 来引入, query 在url中显示参数
              path: "/orderSuccess",
              query: { orderId: res.result.orderId },

              // params要用 name 来引入, params 在url中不显示参数
              // name: "/orderSuccess",
              // parmas: { orderId: res.result.orderId },
            });
          }
        });
    },
