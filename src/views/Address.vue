<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <!-- 默认显示三个地址，用v-for遍历的话，不好实现展开效果，可以通过过滤属性来一次展示三条信息 -->
                <li
                  v-for="(item, index) in addressListFilter"
                  :key="index"
                  :class="{ check: item.addressId == defaultAddress }"
                  @click="selectedAddressId = item.addressId"
                >
                  <dl>
                    <dt>{{ item.userName }}</dt>
          ...
          <div class="next-btn-wrap">
            <!-- 这样写不能传参 -->
            <!-- <router-link class="btn btn--m btn--red" to="/orderConfirm">下一步</router-link> -->
            <!-- 要 v-bind 绑定，然后动态传参 -->
            <router-link
              class="btn btn--m btn--red"
              :to="{
                path: 'orderConfirm',
                query: { addressId: defaultAddress },
              }"
              >下一步</router-link
            >
          </div>
    ...
    <modal :mdShow="isMdShow" @close="closeModal">
      <p slot="message">您确认删除此地址吗？</p>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="delAddress()">确认</a>
        <a class="btn btn--m" href="javascript:;" @click="isMdShow = false"
          >取消</a
        >
      </div>
    </modal>
...
export default {
  ...
  computed: {
    ...
    defaultAddress() {
      ...
      // let d = this.addressList.filter((item) => {
      //   item.isDefault == true;
      // });
      return this.selectedAddressId || d;
    },
  },
  components: {
    NavHeader,
    ...
    setDefault(addressId) {
      axios
        ...
        .then((resp) => {
          ...
          if (res.status == 0) {
            this.init();
            // 如果怕请求接口影响性能，可以遍历addressList改掉isDefalut
          }
        });
    },
    closeModal() {
