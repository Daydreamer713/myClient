<template>
  <div>
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editCart('minu', item)"
                          >-</a
                        >
                        <span class="select-ipt">{{ item.productNum }}</span>
                        <a class="input-add" @click="editCart('add', item)"
                          >+</a
                        >
                      </div>
                    </div>
    ...
    <modal :mdShow="modalConfirm" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="_delItem()">确认</a>
        <a class="btn btn--m" href="javascript:;" @click="modalConfirm = false"
          >关闭</a
        >
      </div>
    </modal>
...
export default {
  data() {
    return {
      ...
      productId: "",
      delItem: "",
      // checkAllFlag: false, 在computed里实时计算的属性就不用在data里声明了
    };
  },
  mounted() {
    this.init();
  ...
  // filters: {
  //   // 局部过滤器，我们还是在 main.js 做全局过滤器的吧
  //   // 过滤器名 : 函数(值)，处理完后 return 新值
  //   currency: currency,
  // },
  computed: {
    //computed 的值只能实时计算，不能再通过其他方法修改值
    checkAllFlag() {
  ...
  methods: {
    ...
    _delItem() {
      axios
        ...
        .then((resp) => {
          ...
          if (res.status == 0) {
            this.modalConfirm = false;
            this.init(); // 待优化：直接遍历this.cartList进行删除，能少调一次接口。
    ...
    checkAll() {
      // this.checkAllFlag 已在 computed 中实时计算，不能再次被赋值
    ...
    checkout() {
      if (this.checkedCount > 0) {
        // 路由有：动态路由、潜逃路由、具名路由，都是通过 router-link 配置的
        this.$router.push({
          //是对 history 进行封装，通过push一个对象进去指定一个path
          path: "/address",
        });
      }
    },
