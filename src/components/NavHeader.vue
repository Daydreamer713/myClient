    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex">
        <div class="navbar-menu-container">
          ...
          <a class="navbar-link" v-if="nickName">{{ nickName }}</a>
          <a
    ...
    >
      <div class="md-modal-inner">
        <div class="md-top">
          ...
          <button class="md-close" @click="loginModalFlag = false">
            Close
          </button>
        </div>
  ...
  methods: {
    checkLogtin() {
      axios.get("/users/checkLogin").then((resp) => {
        let res = resp.data;
        if (res.status == 0) {
          // this.nickName = res.result; // 调接口将用户名取出
          this.$store.commit("updateUserInfo", res.result); // Vuex 控制
          this.getCartCount();
        }
      });
    },
    ...
    getCartCount() {
      axios.get("/users/getCartCount").then((resp) => {
        let res = resp.data;
        this.$store.commit("initCartCount", res.result); // 提交到 vuex 里，其他组件即可自由调用
      });
    },
  },
};
